/* From the description of SHA-256 in Wikipedia */

#include <stdint.h>
#include <stdlib.h>
#include <string.h>

#define rotr(x, n) ((x >> n) | (x << (32 - n)))

static const uint32_t K[64] = {
	0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
	0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
	0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
	0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
	0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
	0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
	0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
	0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

static void statetobuf(const uint32_t *H, uint8_t *buf) {
	size_t i;

	for(i = 0; i < 8; i++) {
		buf[i * 4 + 0] = (H[i] >> 24) & 0xff;
		buf[i * 4 + 1] = (H[i] >> 16) & 0xff;
		buf[i * 4 + 2] = (H[i] >>  8) & 0xff;
		buf[i * 4 + 3] = (H[i]      )& 0xff;
	}
}

static void sha256init(uint32_t *H) {
	H[0] = 0x6a09e667;
	H[1] = 0xbb67ae85;
	H[2] = 0x3c6ef372;
	H[3] = 0xa54ff53a;
	H[4] = 0x510e527f;
	H[5] = 0x9b05688c;
	H[6] = 0x1f83d9ab;
	H[7] = 0x5be0cd19;
}

int newsize(int insize) {
	int size;

	size = insize + 1;
	size += (size % 64 > 56)? 64: 0;
	size += 64 - size % 64;

	return size;
}

static uint8_t *preprocess(const uint8_t *in, int insize, int size) {
	uint8_t *out;
	int i;

	if((out = malloc(size)) == NULL)
		return NULL;

	memcpy(out, in, insize);
	out[insize] = 0x80;
	
	/* We assume size to be 32bit, so we
	   write the first 32 bits as zeroes. */

	for(i = insize + 1; i < size - 4; i++)
		out[i] = 0;
	for(i = 0; i < 4; i++)
		out[size - 4 + i] = ((insize * 8) >> ((3 - i) * 8)) & 0xff;

	return out;
}

static void digest(uint8_t *in, int size, uint32_t H[8]) {
	int i, j, k;
	uint a, b, c, d, e, f, g, h;
	uint W[64], s0, s1, maj, t1, t2, ch;

	sha256init(H);

	for(i = 0; i < size / 64; i++) {
		for(j = 0; j < 16; j++) {
			W[j] = 0;
			for(k = 0; k < 4; k++) {
				W[j] <<= 8;
				W[j] |= in[i * 64 + j * 4 + k];
			}
		}

		for(j = 16; j < 64; j++) {
			s0 = rotr(W[j - 15], 7) ^ rotr(W[j - 15], 18) ^ (W[j - 15] >> 3);
			s1 = rotr(W[j - 2], 17) ^ rotr(W[j - 2], 19) ^ (W[j - 2] >> 10);
			W[j] = W[j - 16] + s0 + W[j - 7] + s1;
		}

		a = H[0];
		b = H[1];
		c = H[2];
		d = H[3];
		e = H[4];
		f = H[5];
		g = H[6];
		h = H[7];

		for(j = 0; j < 64; j++) {
			s0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
			maj = (a & b) ^ (a & c) ^ (b & c);
			t2 = s0 + maj;
			s1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
			ch = (e & f) ^ ((~e) & g);
			t1 = h + s1 + ch + K[j] + W[j];
			
			h = g;
			g = f;
			f = e;
			e = d + t1;
			d = c;
			c = b;
			b = a;
			a = t1 + t2;
		}
		H[0] += a;
		H[1] += b;
		H[2] += c;
		H[3] += d;
		H[4] += e;
		H[5] += f;
		H[6] += g;
		H[7] += h;
	}
}

int sha256(const uint8_t *in, const size_t insize, uint8_t *out) {
	uint8_t *prep;
	size_t size = newsize(insize);
	uint32_t H[8];

	if((prep = preprocess(in, insize, size)) == NULL)
		return 0;

	digest(prep, size, H);
	free(prep);

	statetobuf(H, out);
	return 1;
}
